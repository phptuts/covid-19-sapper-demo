import { c as createCommonjsModule, _ as _typeof, V as commonjsGlobal } from './client.80764277.js';
...
var cacheKeysKey = 'cache_keys';
var expiryTime = 1000 * 60 * 60;
var deleteCacheKeys = function deleteCacheKeys() {
  var cacheKeys = localStorage.getItem(cacheKeysKey);

  if (lodash.isEmpty(cacheKeys)) {
    return;
  }

  JSON.parse(cacheKeys).forEach(function (key) {
    localStorage.removeItem(key);
  });
};
var getCacheData = function getCacheData(cacheKey) {

  if (lodash.isEmpty(localStorage.getItem(cacheKey))) {
    return undefined;
  }

  var _JSON$parse = JSON.parse(localStorage.getItem(cacheKey)),
...
var saveCache = function saveCache(cacheKey, data) {

  var expires = new Date().getTime() + expiryTime;
  ...
  if (!lodash.isEmpty(localStorage.getItem(cacheKeysKey))) {
...
export { lodash as _, deleteCacheKeys as d, getCacheData as g, saveCache as s };
